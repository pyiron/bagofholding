...
class TestBag(unittest.TestCase):
    ...
    def test_bag_info_check(self):
        bag_class.save(42, self.save_name)
        bag_class(self.save_name)
    ...
    def test_version_checking(self):
        ...
        bag_class.save(obj, self.save_name)
        bag = bag_class(self.save_name)
        self.assertEqual(
            np.__version__,
            bag["object"].version,
    ...
    def test_cases(self):
        ...
        global_content = [
            ...
            for obj in [
                ...
                bag_class._unpack_bag_info,  # function -- types.FunctionType
                DRAGON,  # Singleton
            ]
        ]
        reducible_content = [
    ...
    def test_versions_required(self):
        ...
        bag_class.save(obj, self.save_name, require_versions=False)
        reloaded = bag_class(self.save_name).load()
        self.assertEqual(
            reloaded,
            obj,
        ...
        with self.assertRaises(
            ...
            bag_class.save(obj, self.save_name, require_versions=True)

    def test_forbidden_modules(self):
        obj = objects.SomeData()

        bag_class.save(obj, self.save_name, forbidden_modules=())
        reloaded = bag_class(self.save_name).load()
        self.assertEqual(
            reloaded,
            obj,
    ...
    def test_list_paths(self):
        bag_class.save(Parent(), self.save_name)
        paths = bag_class(self.save_name).list_paths()
        self.assertSetEqual(
            {
                "object",
                "object/args",
    ...
    def test_subaccess(self):
        r = Recursing(2)
        bag_class.save(r, self.save_name)
        self.assertEqual(
            r.label,
            bag_class(self.save_name).load("object/state/label"),
    ...
    def test_bad_protocol(self):
        with self.assertRaises(
            ...
            bag_class.save(42, self.save_name, _pickle_protocol=5)
